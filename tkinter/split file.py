...
window.geometry('550x300+550+80')
window.title('Отчет по прямой маркировке')
window.iconbitmap('tkinter\og_img.ico')
...
def ok_button():
    try:
        ...
        for i in range (rol_minint-1,rol_maxint):
            ...
            frame, text='Запустить программу', command=write_and_run).grid(row=(dif+1000), columnspan=4,pady=15)  
...
button_ok = tk.Button(
frame, text='Ввести',command=ok_button,width=8,height=1,foreground='darkgreen').grid(row=7, columnspan=43,padx=10,pady=15)
